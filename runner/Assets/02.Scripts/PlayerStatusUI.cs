...
public class PlayerStatusUI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 이 컴포넌트를 가지는 GameObject가 가지는 컴포넌트들 중에 해당 타입을 찾아서 변환함
        ...
        // ex) RefreshHPBar
        // 1.인라인함수는 접근제핝가 의미 없으므로 private 생략
        // 2.구독하려는 대리자의 형식이 float 파라미터 1개와 void 반환이므로 void 및 float 타임 생략
        // 3. 인라인이므로 이름으로 함수검색할 일 없기때문에 이름 생략
        // 4.구현부 한줄이면 그다음은 반드시 함수 리턴이 일어나야하므로 함수범위 지정 필요없기 때문에 중괄호 생략
        // 5.람다식 명시를 위해서는 반드시 => 추가해주어야한다.
